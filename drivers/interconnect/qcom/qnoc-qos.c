...
#define QOS_SHAPER_EN 			BIT(1)

#define QOSGEN_SHAPING_LO(p, qp)	((p)->offsets[qp] + \
					(p)->regs[QOSGEN_OFF_SHAPING_LO])
#define QOSGEN_SHAPING_HI(p, qp)	((p)->offsets[qp] + \
					(p)->regs[QOSGEN_OFF_SHAPING_HI])
#endif
...
#if defined(CONFIG_SAMSUNG_SBI)
		if (qos->config->shaping_en) {
			//pr_info("Setting shaping en : offset:0x%x value:0x%x 0x%x",
			//	QOSGEN_SHAPING_LO(qos, port),
			//	qos->config->shaping_low,
			//	qos->config->shaping_high);

			regmap_write(node->regmap,
				QOSGEN_SHAPING_LO(qos, port),
				qos->config->shaping_low);

			regmap_write(node->regmap,
				QOSGEN_SHAPING_HI(qos, port),
				qos->config->shaping_high);

			regmap_update_bits(node->regmap,
				QOSGEN_MAINCTL_LO(qos, port),
				QOS_SHAPER_EN, QOS_SHAPER_EN);
		} else {
			regmap_update_bits(node->regmap,
				QOSGEN_MAINCTL_LO(qos, port),
				QOS_SHAPER_EN, 0);
