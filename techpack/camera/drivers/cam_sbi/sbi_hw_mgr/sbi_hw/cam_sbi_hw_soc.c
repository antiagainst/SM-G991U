...
int cam_sbi_soc_enable_resources(struct cam_hw_info *sbi_hw)
{
	...
	if (clk_level < CAM_SVS_VOTE) {
		CAM_DBG(CAM_SBI, "force set_clk svs");
		// force to set min clk as 475 MHz (SVS)
		clk_level = CAM_SVS_VOTE;
	}
	...
	irq_handler_t irq_handler, void *private_data)
{
	...
	if (rc) {
		CAM_ERR(CAM_SBI, "Custom QoS selection failed %d", rc);
...
unregister_cpas:
	if (cam_cpas_unregister_client(soc_private->cpas_handle))
		CAM_ERR(CAM_SBI, "CPAS0 unregistration failed");
