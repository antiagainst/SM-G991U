...
static ssize_t slsi_ts_reg_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size);
static ssize_t slsi_ts_regreadsize_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size);
static inline ssize_t slsi_ts_store_error(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static ssize_t slsi_ts_enter_recovery_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size);
static ssize_t slsi_ts_regread_show(struct device *dev,
		struct device_attribute *attr, char *buf);
static ssize_t slsi_ts_gesture_status_show(struct device *dev,
		struct device_attribute *attr, char *buf);
static inline ssize_t slsi_ts_show_error(struct device *dev,
		struct device_attribute *attr, char *buf);

static DEVICE_ATTR(slsi_ts_reg, (S_IWUSR | S_IWGRP), NULL, slsi_ts_reg_store);
static DEVICE_ATTR(slsi_ts_regreadsize, (S_IWUSR | S_IWGRP), NULL, slsi_ts_regreadsize_store);
static DEVICE_ATTR(slsi_ts_enter_recovery, (S_IWUSR | S_IWGRP), NULL, slsi_ts_enter_recovery_store);
static DEVICE_ATTR(slsi_ts_regread, S_IRUGO, slsi_ts_regread_show, NULL);
static DEVICE_ATTR(slsi_ts_gesture_status, S_IRUGO, slsi_ts_gesture_status_show, NULL);

static struct attribute *cmd_attributes[] = {
	&dev_attr_slsi_ts_reg.attr,
	&dev_attr_slsi_ts_regreadsize.attr,
	&dev_attr_slsi_ts_enter_recovery.attr,
	&dev_attr_slsi_ts_regread.attr,
	&dev_attr_slsi_ts_gesture_status.attr,
	NULL,
};
...
/* for debugging--------------------------------------------------------------------------------------*/
static ssize_t slsi_ts_reg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (size > 0)
		ts->slsi_ts_i2c_write_burst(ts, (u8 *)buf, size);

	input_info(true, &ts->client->dev, "%s: 0x%x, 0x%x, size %d\n", __func__, buf[0], buf[1], (int)size);
	return size;
}

static ssize_t slsi_ts_regread_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct slsi_ts_data *ts = dev_get_drvdata(dev);
	int ret;
	int length;
	...
	do {
		if (remain >= ts->plat_data->i2c_burstmax)
			length = ts->plat_data->i2c_burstmax;
		else
			length = remain;

		if (offset == 0)
			ret = ts->slsi_ts_i2c_read(ts, lv1cmd, &read_lv1_buff[offset], length);
		else
			ret = ts->slsi_ts_i2c_read_bulk (ts, &read_lv1_buff[offset], length);
...
static ssize_t slsi_ts_gesture_status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct slsi_ts_data *ts = dev_get_drvdata(dev);
...
static ssize_t slsi_ts_regreadsize_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct slsi_ts_data *ts = dev_get_drvdata(dev);
...
static ssize_t slsi_ts_enter_recovery_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct slsi_ts_data *ts = dev_get_drvdata(dev);
	...
	if (on == 1) {
		...
		/* AFE Calibration */
		ret = ts->slsi_ts_i2c_write(ts, SLSI_TS_CMD_CALIBRATION_AMBIENT, NULL, 0);
		if (ret < 0)
	...
	memset(data, 0x0, 3);
	ret = ts->slsi_ts_i2c_read(ts, SLSI_TS_READ_ID, data, 3);
	if (ret < 0) {
...
int slsi_ts_raw_device_init(struct slsi_ts_data *ts)
{
	...
	ts->dev = sec_device_create(ts, "slsi_ts");
