...
static long tsp_read(struct slsi_ts_data *ts, struct tsp_ioctl *ioctl_data)
{
	...
	return ts->slsi_ts_i2c_read(ts, reg_addr[0], data, data_len);
}

static long tsp_write(struct slsi_ts_data *ts, struct tsp_ioctl *ioctl_data)
{
	u8 *reg_addr = ioctl_data->reg_addr;
	u16 data_len = ioctl_data->data_len;
	u8 *data = ioctl_data->data;
	...
	/* If register address size is larger than 1 byte, use reg_addr_len */
	return ts->slsi_ts_i2c_write(ts, reg_addr[0], data, data_len);
}
				...
				void __user *p, int compat_mode)
{
	...
	case IOCTL_TSP_LPM:
		...
		ret = ts->plat_data->lpmode(ts, mode);
		if (ret < 0)
	...
	case IOCTL_TSP_PWR:
		...
		ret = ts->plat_data->power(&ts->client->dev, on);
		if (ret < 0)
			snprintf(&ioctl_data.data[0], 5, "FAIL");
		else
			snprintf(&ioctl_data.data[0], 3, "OK");
