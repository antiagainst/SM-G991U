			 ...
			 size_t count, loff_t *ppos)
{
	...
	case FLAG_SELF_MOVE:
		...
		break;
	case FLAG_SELF_MASK:
		make_mass_self_display_img_cmds_HAD(vdd, TX_SELF_MASK_IMAGE, op);
		vdd->self_disp.operation[op].select = true;
		break;
	case FLAG_SELF_ICON:
		make_mass_self_display_img_cmds_HAD(vdd, TX_SELF_ICON_IMAGE, op);
		vdd->self_disp.operation[op].select = true;
		break;
	case FLAG_SELF_GRID:
		...
		break;
	case FLAG_SELF_ACLK:
		make_mass_self_display_img_cmds_HAD(vdd, TX_SELF_ACLOCK_IMAGE, op);
		vdd->self_disp.operation[op].select = true;
		vdd->self_disp.operation[FLAG_SELF_DCLK].select = false;
		break;
	case FLAG_SELF_DCLK:
		make_mass_self_display_img_cmds_HAD(vdd, TX_SELF_DCLOCK_IMAGE, op);
		vdd->self_disp.operation[op].select = true;
