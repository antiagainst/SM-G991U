...
static bool hyp_check_page_ro(u64 va)
{
	...
	fastuh_call(FASTUH_APP_RKP, CMD_ID_TEST_GET_PAR, (unsigned long)va, RKP_PA_WRITE, 0, 0);
	par = *ha1;
...
static int test_case_kernel_range_rwx(void)
{
	...
	struct mem_range test_ranges[] = {
		...
		{((u64)__start_rodata),			((u64) __end_rodata) - ((u64)__start_rodata),	"SRODATA - ERODATA ", true, false},
...
static int __init rkp_test_init(void)
{
	...
	if (!va)
		return -1;
	fastuh_call(FASTUH_APP_RKP, RKP_ROBUFFER_ALLOC, va, 0, 0, 1);
...
{
	fastuh_call(FASTUH_APP_RKP, RKP_ROBUFFER_FREE, (u64)ha1, 0, 0, 1);
