				    ...
				    unsigned long last_size)
{
	...
	if (!IS_ERR_VALUE(paddr)) {
		page = pfn_to_page(__phys_to_pfn(paddr));
...
static int ion_rbin_heap_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_ION_MSM_HEAPS
	rbin_heap->heap.buf_ops = msm_ion_dma_buf_ops;
#endif
	rbin_heap->heap.type = (enum ion_heap_type)ION_HEAP_TYPE_RBIN;
	rbin_heap->heap.name = kstrndup(rmem->name, MAX_HEAP_NAME - 1, GFP_KERNEL);
	rbin_heap->count = rmem->size >> PAGE_SHIFT;
#ifdef CONFIG_ION_MSM_HEAPS
	rbin_heap->dev = &pdev->dev;
	rbin_heap->heap.id = ION_CAMERA_HEAP_ID;
#endif
	...
	ret = ion_device_add_heap(&rbin_heap->heap);
	if (ret) {
		return ret;
	}
...
static int ion_rbin_heap_remove(struct platform_device *pdev)
{
	...
	ion_device_remove_heap(&rbin_heap->heap);
	ion_rbin_heap_destroy_pools(rbin_heap->pools);
	kfree(rbin_heap);
