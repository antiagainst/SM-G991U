...
enum cam_sbi_reg_cmd {
	...
	cam_sbi_reg_cmd_task2_error_action,
	cam_sbi_req_cmd_update_ssm_info,
	cam_sbi_req_cmd_ssm_record_start,
	cam_sbi_req_cmd_enable_vvalidtime_log,
};
...
static int cam_sbi_hw_mgr_start(void *hw_mgr_priv, void *hw_start_args)
{
	...
	struct cam_ife_csid_epd_update_args s_epd_config = { .epd_supported = 1 };
	...
	/* set current csid config information to CUSTOM CSID HW */
...
start_only:
	...
	if (!ctx_manager->is_ssm_recording_instance)
	{
...

		{
			...
			/* Init custom hw here */
			cam_sbi_hw_mgr_init_register_set(hw_mgr, args, args_size, cam_sbi_reg_cmd_init);
			...
			if (cmd->is_ssm) {
				...
				cam_sbi_hw_mgr_invoke_writereg_cmd(
					hw_mgr, (void *)(&ssm_info), cam_sbi_reg_cmd_base_addr_upd_scenario_4);
				...
				cam_sbi_hw_mgr_invoke_writereg_cmd(hw_mgr,
					(void *)(&g_sbi_hw_mgr.debugfs_entry.vvalid_time_debug),
					cam_sbi_req_cmd_enable_vvalidtime_log);

				/* set to dma address */
				cam_sbi_hw_mgr_invoke_writereg_cmd(
					hw_mgr, (void *)(&scratch_iova), cam_sbi_reg_cmd_base_addr_upd_scenario_1);
				CAM_INFO(CAM_SBI, "SBI base addr update. Preview buffer:0x%llx",
					scratch_iova);
			}

			cam_sbi_hw_mgr_dump_specific_reg(hw_mgr, 0x278);// 0x1fc); SBIv2 updated
			cam_sbi_hw_mgr_dump_specific_reg(hw_mgr, 0xc08);//0x608);	//wdma0 base address
...
cam_sbi_hw_mgr_get_csid_res_type(
	...
	int32_t write_reg_cmd)
{
	...
	case cam_sbi_reg_cmd_task2_setup:
		...
		rc = hw_device->hw_intf.hw_ops.process_cmd(hw_device->hw_intf.hw_priv,
			...
			sizeof(uint32_t));
		break;
	case cam_sbi_reg_cmd_task2_error_action:
		sbi_ctl_cmd = CAM_SBI_HW_CMD_WRITEREG_TASK2_ERROR_ACTION;
		rc = hw_device->hw_intf.hw_ops.process_cmd(hw_device->hw_intf.hw_priv,
			sbi_ctl_cmd, val,
			sizeof(uint32_t));
		break;
	...
	case cam_sbi_req_cmd_enable_vvalidtime_log:
		rc = hw_device->hw_intf.hw_ops.process_cmd(hw_device->hw_intf.hw_priv,
			CAM_SBI_HW_CMD_ENABLE_VVALID_TIME_LOG, val,
			sizeof(uint32_t));
...
static int cam_sbi_set_vvalid_debug(void *data, u64 val)
{
	g_sbi_hw_mgr.debugfs_entry.vvalid_time_debug = val;
	CAM_DBG(CAM_SBI, "set vvalid_time Debug value :%lld", val);
	return 0;
}

static int cam_sbi_get_vvalid_debug(void *data, u64 *val)
{
	*val = g_sbi_hw_mgr.debugfs_entry.vvalid_time_debug;
	CAM_DBG(CAM_SBI, "vvalid time Debug value :%lld",
		g_sbi_hw_mgr.debugfs_entry.vvalid_time_debug);
	return 0;
}
...
DEFINE_SIMPLE_ATTRIBUTE(vvalid_time_debug,
	cam_sbi_get_vvalid_debug,
	cam_sbi_set_vvalid_debug, "%16llu");
...
static int cam_sbi_hw_mgr_reset(void *hw_mgr_priv, void *hw_reset_args)
{
	...
	if (hw_device->hw_intf.hw_ops.reset) {
		reset_cmd.reset_type = CAM_SBI_HW_RESET_TYPE_SBI0_RESET;// CAM_SBI_HW_RESET_TYPE_CORE0_RESET;// no meaning
...
static int cam_sbi_mgr_create_debugfs_entry(void)
{
	...
	if (!debugfs_create_file("custom_csid_debug",
		...
		CAM_ERR(CAM_SBI, "failed to create cam_custom_csid_debug");
		goto err;
	}

	if (!debugfs_create_file("vvalid_time_debug",
		0644,
		g_sbi_hw_mgr.debugfs_entry.dentry, NULL,
		&vvalid_time_debug)) {
		CAM_ERR(CAM_SBI, "failed to create vvalid_time_debug");
		goto err;
	}
...
void cam_req_mgr_process_workq_cam_sbi_worker(struct work_struct *w)
{
