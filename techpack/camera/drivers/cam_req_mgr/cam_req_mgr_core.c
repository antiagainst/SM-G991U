	...
	struct cam_req_mgr_connected_device **failed_dev)
{
	...
	apply_req.recovery_apply = false;
	if (link->retry_cnt > 0) {
	...
	for (i = 0; i < link->num_devs; i++) {
		...
		if (dev) {
			...
			idx = apply_data[pd].idx;
			slot = &dev->pd_tbl->slot[idx];
			apply_req.recovery_apply = in_q->slot[idx].err_reported;
	...
	if (rc < 0) {
		...
		if (check_retry_cnt) {
			...
			CAM_WARN(CAM_CRM,
				...
				in_q->rd_idx);
			link->wq_retry_cnt++;
			if (link->wq_retry_cnt == (MAXIMUM_RETRY_ATTEMPTS + 5)) {
				__cam_req_mgr_notify_error_on_link(link, dev);
				link->wq_retry_cnt = 0;
			}
		}
...
int cam_req_mgr_process_error(void *priv, void *data)
{
	...
	if (err_info->error == CRM_KMD_ERR_BUBBLE) {
		...
		if (idx < 0) {
			...
			in_q->rd_idx = idx;
			in_q->slot[idx].status = CRM_SLOT_STATUS_REQ_ADDED;
			in_q->slot[idx].err_reported = true;
			if (link->sync_link[0]) {
